...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text('سلام! برای دانلود ویدیو، لینک ویدیو را برای من بفرستید.')
...
async def download_video(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if url.startswith('http'):
        ...
        if response.status_code == 200:
            ...
            await context.bot.send_message(chat_id=chat_id, text='صفحه قابل دسترسی نیست.')
    else:
        await context.bot.send_message(chat_id=chat_id, text='لطفاً لینک ویدیو را بفرستید.')
    ...
    try:
        ...
        if video_response.status_code == 200:
            ...
            with open(file_name, 'rb') as video_file:
                await context.bot.send_video(chat_id=chat_id, video=video_file, timeout=120)

            await context.bot.send_message(chat_id=chat_id, text='ویدیو با موفقیت دانلود و ارسال شد.')
        else:
            await context.bot.send_message(chat_id=chat_id, text='مشکلی در دانلود ویدیو پیش آمده است.')
    except Exception as e:
        print(f"Error sending video: {e}")
        await context.bot.send_message(chat_id=chat_id, text='مشکلی در ارسال ویدیو به وجود آمده است.')
    finally:
