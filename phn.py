...
def upload_file(file_path):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
                return data.get('link')  # لینک دانلود فایل
            else:
                print(f"آپلود موفق نبود: {data}")
        else:
            print(f"خطا در اتصال به سرور، کد وضعیت: {response.status_code}")
...
# دانلود، آپلود و ارسال لینک ویدیو
